...
router.post('/', async(req, res) => {
  try {
    ...
    if(!category_id) {
      return res
      .status(400)
      .json({ error: "Category id is mandatory."});
    }

    if(!value) {
      return res
      .status(400)
      .json({ error: "Value is mandatory."});
    }

    if(!title || title.length < 3) {
      return res
      .status(400)
      .json({ error: "Title is mandatory and should have more than 3 characters."});
    }

    if(!date || date.length != 10) {
      return res
      .status(400)
      .json({ error: "Date id is mandatory and should be in the format yyyy-mm-dd."});
    }
...
router.get('/', async(req, res) => {
  try {
    ...
    const values = [userQuery.rows[0], initDate, finDate]
